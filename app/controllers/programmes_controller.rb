...
# GET /programmes
  # GET /programmes.json
  def index
    @programmes = Programme.paginate(:per_page => 15, :page => params[:page])
  ...
  def create
    ...
    respond_to do |format|
      if @programme.save
        format.html { redirect_to @programme, notice: 'Course programme was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @programme.update_attributes(params[:programme])
        format.html { redirect_to @programme, notice: 'Course programme was successfully updated.' }
